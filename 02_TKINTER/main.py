from tkinter import Tk, Label, Button, Entry, StringVar, Frame, Radiobutton
...
window = Tk()
window.title("Form Pendaftaran Siswwa Ignatius Global School")
...
day_var = StringVar()
day_combobox = Combobox(birth_date_frame, width=3)
day_combobox['values'] = list(range(1, 32))
day_combobox.current(datetime.now().day-1)
day_combobox.grid(row=0, column=0, stick="nsew")

month_var = StringVar()
month_combobox = Combobox(birth_date_frame, width=9)
...
jk_label.grid(row=5, column=1, sticky="w")
